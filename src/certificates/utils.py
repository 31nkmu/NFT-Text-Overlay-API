...
def render_image(image: bytes, params: schemas.InsertTextParams):  # noqa: C901
    ...
    for _, field_data in params.fields.items():
        ...
        x1, y1 = coordinates[1]  # нижний правый угол
        field_width = x1 - x0
        field_height = y1 - y0
...
def create_an_image_buffer(rendered_image: models.Certificate):
...
def generate_filename(original: models.PublicTemplate, db: Session) -> str:
    ...
    owner = db.query(User).get(original.owner.id)
    if not owner:
    ...
    while True:
        ...
        if not db.query(models.Certificate)\
